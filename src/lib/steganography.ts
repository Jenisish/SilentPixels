export class SteganographyService {
  ...
  private static SIGNATURE = "SPv2"; // Convert text to binary (improved for UTF-8)
  ...
  private static async encryptMessage(
    ...
    const signatureBinary = this.textToBinary(this.SIGNATURE);
    const saltBinary = this.arrayBufferToBinary(salt.buffer);
    const ivBinary = this.arrayBufferToBinary(iv.buffer);
  ...
  private static async decryptHiddenData(
    hiddenDataBinary: string,
    key: string
  ): Promise<string> {
    ...
    const saltBinary = hiddenDataBinary.slice(
      signatureLength,
      signatureLength + 128
    );
    const ivBinary = hiddenDataBinary.slice(
      signatureLength + 128,
      signatureLength + 128 + 96
    );
  ...
  private static async encodeDocument(
    file: File,
    lengthBinary: string,
    hiddenDataBinary: string
  ): Promise<Blob> {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        ...
        for (let i = 0; i < hiddenDataBinary.length; i++) {
          uint8Array[i + 32] =
            (uint8Array[i + 32] & 254) | parseInt(hiddenDataBinary[i]);
        }
  ...
  private static async extractHiddenDataFromDocument(
    file: File
  ): Promise<string> {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
  ...
  // Video encoding
  private static async encodeVideo(
    file: File,
    lengthBinary: string,
    hiddenDataBinary: string
  ): Promise<Blob> {
    const START_OFFSET = 4096; // Offset to skip potential header bytes
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = async (e) => {
        const arrayBuffer = e.target?.result as ArrayBuffer;
        ...
        // Check if the video is large enough to embed length and some data
        if (uint8Array.length < START_OFFSET + 32) {
          reject(new Error("Video file is too small to embed data"));
          return;
        }

        // Calculate available capacity for hidden data
        const availableBytes = uint8Array.length - START_OFFSET - 32;
        if (hiddenDataBinary.length > availableBytes) {
          reject(new Error("Message is too long for this video file"));
          return;
        }

        // Embed length starting at START_OFFSET
        for (let i = 0; i < 32; i++) {
          const index = START_OFFSET + i;
          uint8Array[index] =
            (uint8Array[index] & 254) | parseInt(lengthBinary[i]);
        }

        // Embed hidden data after length
        for (let i = 0; i < hiddenDataBinary.length; i++) {
          const index = START_OFFSET + 32 + i;
          uint8Array[index] =
            (uint8Array[index] & 254) | parseInt(hiddenDataBinary[i]);
        }
  ...
  private static async extractHiddenDataFromVideo(file: File): Promise<string> {
    const START_OFFSET = 4096; // Must match encodeVideo offset
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = (e) => {
        const arrayBuffer = e.target?.result as ArrayBuffer;
        ...
        // Check if the video is large enough to contain embedded data
        if (uint8Array.length < START_OFFSET + 32) {
          reject(new Error("Video file is too small to contain embedded data"));
          return;
        }
        ...
        for (let i = 0; i < 32; i++) {
          const index = START_OFFSET + i;
          lengthBinary += (uint8Array[index] & 1).toString();
        }
        const totalLength = parseInt(lengthBinary, 2);

        // Validate length
        if (totalLength > uint8Array.length - START_OFFSET - 32) {
          reject(new Error("Invalid hidden data length"));
          return;
        }
        ...
        for (let i = 0; i < totalLength; i++) {
          const index = START_OFFSET + 32 + i;
          if (index >= uint8Array.length) break;
          hiddenDataBinary += (uint8Array[index] & 1).toString();
        }
  ...
  // Audio encoding
  private static async encodeAudio(
    file: File,
    lengthBinary: string,
    hiddenDataBinary: string
  ): Promise<Blob> {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          for (let i = 0; i < 32; i++) {
            modifiedData[i] =
              Math.floor(channelData[i] * 1000) / 1000 +
              parseInt(lengthBinary[i]) * 0.0001;
          }

          for (let i = 0; i < hiddenDataBinary.length; i++) {
            modifiedData[i + 32] =
              Math.floor(channelData[i + 32] * 1000) / 1000 +
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          for (
            let i = 32;
            i < totalLength + 32 && i < channelData.length;
            i++
          ) {
  ...
  static async encode(
    ...
    let hiddenDataBinary = key
      ? await this.encryptMessage(message, key)
      : this.textToBinary(message);
